...
public class OrganizationDeduplicationExperiments {
...
//        String basePath = "/Users/paolosottovia/Downloads/extracted_data_17_03_2018/";
        ...
        List<DataSet<Record, Attribute>> datasetsCH = new ArrayList<>();
        List<DataSet<Record, Attribute>> datasetsAT = new ArrayList<>();

        List<DataSet<Record, Attribute>> datasetsSL = new ArrayList<>();
        datasetsIT.add(openAireFiltered);
        ...
        if (includeOrcid) {
            DataSet<Record, Attribute> itORCID = filterDatasetByCountryCode(orcid, "IT", orcidIdToRemove);
            System.out.println("Number of itORCID: " + itORCID.get().size());
            datasetsIT.add(itORCID);
        }

        datasetsFR.add(openAireFiltered);
        datasetsFR.add(scanr);
        if (includeOrcid) {
            DataSet<Record, Attribute> frORCID = filterDatasetByCountryCode(orcid, "FR", orcidIdToRemove);
            System.out.println("Number of frORCID: " + frORCID.get().size());
            datasetsFR.add(frORCID);
        }
        datasetsFR.add(grid);
        datasetsFR.add(p3);
        ...
        datasetsCH.add(grid);
        datasetsCH.add(openAireFiltered);
        datasetsCH.add(p3);

        datasetsAT.add(openAireFiltered);
        datasetsAT.add(grid);
        datasetsAT.add(fwf);


        datasetsSL.add(sicris);
        datasetsSL.add(openAireFiltered);
        datasetsSL.add(grid);

        ALL_DATASETS.add(datasetsIT);
        ALL_DATASETS.add(datasetsFR);
        ALL_DATASETS.add(datasetsCH);
        ALL_DATASETS.add(datasetsAT);
        ALL_DATASETS.add(datasetsSL);
...
//        System.out.println("RUN DISTANCE BASED ================================================================");
    ...
    public static void runStandardEntityMatchingNOCLUSTERING(List<DataSet<Record, Attribute>> dataSets, String mode, Map<String, Record> idRecords, Map<String, String> idSources, Map<String, Set<String>> idOtherCandidates, boolean includeOrcid, Graph<String, DefaultEdge> gSTART,String label, boolean TEST) {
...
//        EdgeFactory<String,DefaultEdge> factory = new
        ...
        try {
            ...
            RecordComparatorLevenshtein lab = new RecordComparatorLevenshtein(Organization.LABEL, Organization.LABEL);
            city.setLowerCase(true);

            RecordComparatorLevenshtein address = new RecordComparatorLevenshtein(Organization.ADDRESS, Organization.ADDRESS);
            link.setLowerCase(true);
...
//            }
            ...
            writeMatches("matches_"+label+".tsv", allCorrespondances);
...
//                ConnectivityInspector<String, DefaultEdge> cinsp = new ConnectivityInspector<>(matchGraph1);
            ...
            writeCorrespondence("correspondenceOrganizations_" + mode + "_"+label+".tsv", finalMatches);
    ...
    public static void runDistanceBasedEntityMatching(List<DataSet<Record, Attribute>> dataSets, String mode, Map<String, Record> idRecords, Map<String, String> idSources, Map<String, Set<String>> idOtherCandidates, boolean includeOrcid, Graph<String, DefaultEdge> gSTART,String label, boolean TEST) {
...
//        EdgeFactory<String,DefaultEdge> factory = new
        ...
        try {
            ...
            matchingRule.addComparator(link, 0.25d);
            matchingRule.addComparator(lab, 0.25d);
            matchingRule.addComparator(address, 0.25d);
...
//
    ...
    public static void runStandardEntityMatching(List<DataSet<Record, Attribute>> dataSets, String mode, Map<String, Record> idRecords, Map<String, String> idSources, Map<String, Set<String>> idOtherCandidates, boolean includeOrcid, Graph<String, DefaultEdge> gSTART, boolean TEST) {
