...
public class RuleBasedMatchingAlgorithm<RecordType extends Matchable, SchemaElementType extends Matchable, CorrespondenceType extends Matchable> implements MatchingAlgorithm<RecordType, CorrespondenceType> {
	...
	public void run() {
		...
		// compare the pairs using the matching rule in parallel
		Processable<Correspondence<RecordType, CorrespondenceType>> p = new ParallelProcessableCollection<Correspondence<RecordType, CorrespondenceType>>(allPairs.get());

		Processable<Correspondence<RecordType, CorrespondenceType>> result = p.map(rule);
